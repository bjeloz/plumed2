  ...
  namespace colvar{
    ...
    */
      ...
      isnotscaled(false)
    {
      ...
      parse("NST",Na_st_permol);  //get number of solute atoms per molecule
      ...
      parse("DF",iD_F); //optionally one can set the force distance from the interface manually
	...
	zright=fix_int;
      ...
      //fdbg << "Interface "<< zleft << "\t" << zright << endl; //DBG
	...
	for(int i=rank; i<N_sv; i+=stride){
	  ...
	  //fdbg << i << "\t" << zout*w_out << "\t" << zin*w_in << "\t" << n_lx << "\t";
	...
	//fdbg << "Solute computing: Mass = " << M_st <<endl;
