...
//+ENDPLUMEDOC
    ...
    public:
      ...
      double zetac, zetal, sigmac, sigmal, sigmac2, sigmal2; // values for kernel function
      double kval, dkval;  // initialize kernel function and its derivative
      double kernel(double, double&);  // kernel function
	...
	vector<Vector> pos2;      // POSITIONS NEIGHBORING MOLECULES
      ...
      //      void GsmacInterval_newlist(vector<AtomNumber> &list, vector<Vector> &grid, GsmacIntervallist_s &GsmacIntervallist);     // NEW VERLIST
      //      void GsmacInterval_checklist(vector<AtomNumber> &list, vector<Vector> &grid, GsmacIntervallist_s &GsmacIntervallist);   // CHECK THE VERLIST

      void GsmacInterval_newlist(vector<AtomNumber>&, GsmacIntervallist_s &GsmacIntervallist);     // NEW VERLIST
      void GsmacInterval_checklist(vector<AtomNumber>&, GsmacIntervallist_s &GsmacIntervallist);   // CHECK THE V
...
double GsmacInterval::kernel(double z, double &dkval) { 
  // switching function to limit action of Gsmac CV

  double kval;

  // transform input values into fractional coordinates
  double zbox = getBox()[2][2];
  z = z/zbox;

  // get value of switching function in dependence of z
  if ( (z < zetac - 3*sigmac) || (zetal + 3*sigmal < z) ) {
    kval = 0;
    dkval = 0;
  } else if (z < zetac) {
    kval = std::exp(-((z-zetac)*(z-zetac))/(2*sigmac2));
    dkval = -(z-zetac)/(2*sigmac2) * kval;
  } else if (zetal < z) {
    kval = std::exp(-((z-zetal)*(z-zetal))/(2*sigmal2));
    dkval = -(z-zetal)/(2*sigmal2) * kval;
  } else {
    kval = 1;
    dkval = 0;
  }
...
void GsmacInterval::calculate()
{
  ...
  if(GsmacIntervallist.step==0) GsmacInterval_newlist(center1,GsmacIntervallist); // CHECK IF NEIGH LIST HAVE TO BE CONSTRUCTED
  GsmacIntervallist.step=1; GsmacInterval_checklist(center1,GsmacIntervallist);   // CALL NEIGH LIST IN CASE
  ...
  for(unsigned int i=rank;i<mols;i+=stride) {                   // SUM OVER MOLECULES
    ...
    kval = kernel(zpos, dkval);   // calculate value of kernel function kval and its derivative dkval
    ...
    if(n>0.){           // THESE PARTS ARE USEFUL IN THE CALCULATION OF THE DERIVATIVES
      ...
      S2 = -angtot / n;      
      S3 = rho*angtot/(n);
      cv_val += kval*S3;     // SUM THE TOTAL CV 
    }else{
      S1 = 0.;
      S2 = 0.;
    }
	...
	double temp = ( (omega[j]  +  S2) * rho / n + S1) * df[j][ix];
      	deriv[i][ix]       += temp*kval + S3*dkval;
      	deriv[index_j][ix] -= temp*kval + S3*dkval;
        // BOX DERIVATIVES
        dist=pbcDistance(getPosition(i),getPosition(index_j));    // DISTANCE BETWEEN THEM
 	for(unsigned jx=0;jx<m;jx++) {
      		 virial[ix][jx] += (temp*kval + S3*dkval) * dist[jx];
	}
	// START
	double temp1 = rho * f[j] * domega1[j][ix] / n * kval;
	double temp2 = rho * f[j] * domega2[j][ix] / n * kval;
...
void GsmacInterval::GsmacInterval_newlist(vector<AtomNumber> &list1, GsmacIntervallist_s &GsmacIntervallist)
{
  ...
  rank=comm.Get_rank(); //Rank of pr
    ...
    //    comm.Sum(GsmacIntervallist.nn);
    //    comm.Sum(GsmacIntervallist.ni[0]);
}
