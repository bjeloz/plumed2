...
//+PLUMEDOC MCOLVAR INTERFACEACTIONS
/*
...
class InterfaceActions : public MultiColvarBase {
private:
  bool use_sf;
...
public:
  ...
  explicit InterfaceActions(const ActionOptions&);
...
PLUMED_REGISTER_ACTION(InterfaceActions,"INTERFACEACTIONS")

void InterfaceActions::registerKeywords( Keywords& keys ) {
...
InterfaceActions::InterfaceActions(const ActionOptions&ao):
  Action(ao),
  MultiColvarBase(ao),
  use_sf(false)
{
...
double InterfaceActions::calculateWeight( const unsigned& taskCode, const double& weight, AtomValuePack& myatoms ) const {
...
double InterfaceActions::compute( const unsigned& tindex, AtomValuePack& myatoms ) const {
  Vector dij=getSeparation( myatoms.getPosition(0), myatoms.getPosition(2) );
  Vector dik=getSeparation( myatoms.getPosition(0), myatoms.getPosition(1) );
